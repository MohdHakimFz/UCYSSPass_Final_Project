...
| `ends_after` | Events that have not ended by this time (ISO date-time): the ones still to come **and** the ones running now. The apps use this for the public list, so an event that has started is still shown until it ends |
...
| Resend email API, or Brevo email API (chosen with `MAIL_API_DRIVER`) | Confirmation, cancellation, waitlist-promotion, reminder and announcement emails, and password reset codes | `EmailService`, logged in `notifications.provider_response` |
...
The rest of the app sends through one `EmailService`, so the email service is a setting: `MAIL_API_DRIVER=resend` (default) or `brevo`. While Resend is in its test mode it only delivers to the address of the Resend account; to email every guest with Resend, a sending domain has to be verified and set as `RESEND_FROM_EMAIL`. Brevo needs only a verified **sender address** (`BREVO_FROM_EMAIL`) and an API key (`BREVO_API_KEY`), and can then email anyone. Either way the raw answer is stored with the email in `notifications.provider_response`. Addresses on domains kept for tests (`.test`, `example.com` and the like) can never receive mail, so they are recorded as `skipped` instead of being sent (`MAIL_SKIP_RESERVED`), which keeps test data from using up the daily allowance or causing bounces.
