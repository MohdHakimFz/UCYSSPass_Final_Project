...
- [ ] **Statistik.** Halaman **My events** (organiser) dan **Overview** (admin) ada panel **Attendance** (untuk event yang sudah bermula, termasuk yang sedang berjalan): "X of Y who registered came (Z%)", dan bilangan "did not attend" (tempah tetapi tak hadir, hanya dikira selepas event tamat). 📸
- [ ] **Sijil.** Butang hanya muncul selepas event **tamat** (sebelum itu ada nota "ready once the event ends"). Customer yang hadir: pada My passes ada **Download certificate**; ia memuat turun PDF dengan nama, event, tarikh, dan nombor sijil. 📸
